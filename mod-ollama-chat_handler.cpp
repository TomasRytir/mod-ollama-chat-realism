    ...
    for (Player* bot : finalCandidates)
    {
        ...
        std::thread([botGuid, senderGuid, prompt, sourceLocal, channelId = (channel ? channel->GetChannelId() : 0)]() {
            try {
                ...
                // Simulate typing delay (3.5 chars/sec casual speed)
                float charsPerSec = 3.5f;
                int delayMs = static_cast<int>((response.length() / charsPerSec) * 1000);

                // Clamp to avoid too short or too long wait
                delayMs = std::clamp(delayMs, 500, 8000); // between 0.5s and 8s max
                ...
                if (IsContentInappropriateForLevel(response, botPtr->GetLevel()))
                {
                    LOG_INFO("server.loading", "Response '{}' is inappropriate for bot level {}. Regenerating...", response, botPtr->GetLevel());

                    std::string fallbackPrompt = GenerateBotPrompt(botPtr, "Regenerate a neutral helpful response.", senderPtr); // fallback neutral prompt
                    std::future<std::string> fallbackFuture = SubmitQuery(fallbackPrompt);
                    std::string fallbackResponse = fallbackFuture.get();
...
//     )";
    ...
    {  
        ...
        // Override personality to neutral for fallback or help-style questions
        if (playerMessage == "Regenerate a neutral helpful response." || IsInformationalQuestion(playerMessage))
        {
            personality = static_cast<BotPersonalityType>(-1);
        }
        ...
        std::string prompt = fmt::format(
            ...
            "Your responses must sound like those of a real human player: natural, brief, relevant, and in-character for a WoW environment. "
